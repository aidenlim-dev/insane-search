...
<img src="assets/hero.png" width="860" alt="シネマティックな分割画面：左は 403 Forbidden・CAPTCHA・WAF でブロックされたサイト。中央から砕け、右で AIOFFICE-SearchPro が突破し、API キーなしで @claudeai（Claude Code Plugins）の実際の公開ポストを出典付きで取得する。">
...
**AI エージェント（Claude Code 自身を含む）にインストールを任せる場合：** 上記のスラッシュコマンドは対話型 UI 専用のため、エージェントからは実行できません。代わりにターミナルで非対話型 CLI を実行させてください：
...
その後、Claude Code を再起動するか `/reload-plugins` を実行するとプラグインが読み込まれます。

検証はリポジトリを**新たにクローンせず**、インストール済みコピーで doctor を直接実行してください：
...
解析用に命中した元 HTML が必要な場合は、成功した同じ呼び出しで保存します：
...
覚えるコマンドはありません。いつも通り Claude Code に話しかければ、フェッチがブロックされた瞬間に AIOFFICE-SearchPro が割り込みます。

## 💬 使ってみる

普通に頼むだけ——ブロックされた瞬間に AIOFFICE-SearchPro が動きます：

> *"Reddit で Claude Code の反応を探して、人気スレッドを要約して。"*
> *"X で AIOFFICE-SearchPro に関する投稿を検索して。"*
> *"この YouTube 動画を要約して。"*
...
- **システム Python を変更しない** —— 初回利用時にプラグイン専用 venv を作成し、`curl_cffi`、`yt-dlp`、解析用依存関係を入れる。API キーも登録も不要。

## 🆚 デフォルト Claude Code vs `+ AIOFFICE-SearchPro`

| こんなとき… | Claude Code 単体 | `+ AIOFFICE-SearchPro` |
| :--- | :--- | :--- |
...
AIOFFICE-SearchPro は**公開コンテンツのリーダー**であり、認証を回避する手段ではありません。
...
**GitHub 直接インストール:** [aidenlim-dev/AIOFFICE-SearchPro](https://github.com/aidenlim-dev/AIOFFICE-SearchPro)
