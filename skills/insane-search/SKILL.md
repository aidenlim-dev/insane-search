...
3. `must_invoke_playwright_mcp=false` — true면 **Claude가 세션에서 직접** MCP Playwright를 돌린 뒤에만 통과: `browser_navigate` → `browser_network_requests`로 내부 `/api`·`/graphql`·`.json` 엔드포인트 탐지 → 그 URL을 `bash "${CLAUDE_PLUGIN_ROOT}/setup/run-engine.sh"`로 재호출(API는 WAF가 얕음); 또는 `browser_snapshot`으로 렌더된 HTML 회수. (engine은 로컬 Node Chrome만 띄울 수 있고 MCP는 못 돌리므로, MCP는 **구조적으로** 에이전트의 몫이다.)
...
3. 식별된 JSON API URL을 `bash "${CLAUDE_PLUGIN_ROOT}/setup/run-engine.sh" <API_URL>`로 재호출 (백그라운드 engine과는 별개 호출). 대부분 API 레이어는 페이지 HTML보다 WAF 보호가 얕아 curl_cffi로 바로 수집됨
...
- **단일 진입점**: 설치된 플러그인에서는 일반 웹 페이지를 항상 `bash "${CLAUDE_PLUGIN_ROOT}/setup/run-engine.sh" <URL>`로 처리한다. 개발 환경에서는 `python3 -m engine <URL>` 또는 `from engine import fetch; fetch(...)`도 가능하다.
...
`fetch()`는 단일 API이지만 내부는 phase로 나뉘어 있다. 설치된 플러그인에서는 `setup/run-engine.sh`가 격리 venv를 준비한 뒤 `python -m engine`을 실행한다. `result.trace`에서 각 시도를 확인할 수 있다.
...
최초 호출 시 `setup/run-engine.sh`가 플러그인 전용 venv(`~/.cache/insane-search/venv`, `INSANE_SEARCH_VENV`로 재정의 가능)를 만들고 필요 패키지를 설치한다. 시스템 Python은 건드리지 않는다. **curl_cffi는 0.15.0 이상**을 요구한다 — 0.15부터
...
> `bash "${CLAUDE_PLUGIN_ROOT}/setup/run-engine.sh" "<URL>"` 하나면 Phase 0 라우터(`engine/phase0.py`)가 **격자보다 먼저** 공식 경로를 시도한다 —
