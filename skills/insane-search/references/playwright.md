...
# 템플릿 로컬 의존성 + 시스템 Chrome + Playwright MCP
...
| `needs_real_tls_stack` only | Approach 2 (real_chrome) | F5 BIG-IP 등 TLS 지문 대응 필요 |
| 둘 다 없음 | 보통 curl 체인에서 해결. fallback까지 가면 안전 기본값으로 Approach 2 | 미분류 챌린지 |
