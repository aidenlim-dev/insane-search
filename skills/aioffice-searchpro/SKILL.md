...
3. `must_invoke_playwright_mcp=false` — true면 **Claude가 세션에서 직접** MCP Playwright를 돌린 뒤에만 통과: `browser_navigate` → `browser_network_requests`로 내부 `/api`·`/graphql`·`.json` 엔드포인트 탐지 → 그 URL을 OS-native `run-engine` wrapper로 재호출(API는 WAF가 얕음); 또는 `browser_snapshot`으로 렌더된 HTML 회수. (engine은 로컬 Node Chrome만 띄울 수 있고 MCP는 못 돌리므로, MCP는 **구조적으로** 에이전트의 몫이다.)
...
3. 식별된 JSON API URL을 OS-native `run-engine` wrapper로 재호출 (백그라운드 engine과는 별개 호출). 대부분 API 레이어는 페이지 HTML보다 WAF 보호가 얕아 curl_cffi로 바로 수집됨
...
- **단일 진입점**: 설치된 플러그인에서는 일반 웹 페이지를 항상 OS-native wrapper로 처리한다. Windows는 `setup/run-engine.ps1`, macOS/Linux/Git Bash는 `setup/run-engine.sh`를 쓴다. 개발 환경에서는 `python3 -m engine <URL>` 또는 `from engine import fetch; fetch(...)`도 가능하다.
- **편향 금지**: `engine/**`, `waf_profiles.yaml`에 특정 사이트 하드코딩 금지.
- **힌트는 런타임에만**: 사이트 고유 정보는 CLI/`user_hint` 경유.

## Codex 호환 — CLAUDE_PLUGIN_ROOT가 없는 하네스에서

이 스킬은 Claude Code와 Codex에서 동일하게 동작한다. Codex처럼 `CLAUDE_PLUGIN_ROOT` 환경변수가 없는 하네스에서는 **이 SKILL.md의 위치 기준으로 플러그인 루트를 해석**하라: 이 파일은 `<PLUGIN_ROOT>/skills/aioffice-searchpro/SKILL.md`에 있으므로 루트는 두 단계 위다. 이 문서의 모든 `${CLAUDE_PLUGIN_ROOT}` / `$env:CLAUDE_PLUGIN_ROOT`를 그 경로로 치환하면 된다 (예: wrapper는 `<PLUGIN_ROOT>/setup/run-engine.sh` 또는 `<PLUGIN_ROOT>\setup\run-engine.ps1`).

도구 이름 매핑 — R1을 포함한 모든 규칙은 동일하게 적용된다(차단 감지 시 즉흥 fetch 금지, wrapper 실행):

| 이 문서의 표기 | Codex 대응 |
|---|---|
| WebFetch | `web.open` |
| WebSearch | `web.search_query` |
| Bash / PowerShell 도구 | `shell_command` |

업데이트(사용자가 요청할 때만): `codex plugin marketplace upgrade aioffice-searchpro-marketplace` 후 `codex plugin add aioffice-searchpro@aioffice-searchpro-marketplace`, 그리고 새 세션을 연다.
